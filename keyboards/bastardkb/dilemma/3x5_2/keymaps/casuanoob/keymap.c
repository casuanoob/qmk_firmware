...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_nAPTmak] = LAYOUT_dilemma(nAPTmak_split_3x5_2),
  [_CLMKdh] = LAYOUT_dilemma(COLEMAKdhm_split_3x5_2),
  [_NRSTex] = LAYOUT_dilemma(NRSTex_split_3x5_2),
  [_ADEPT] = LAYOUT_dilemma(ADEPT_split_3x5_2),
  [_APTm] = LAYOUT_dilemma(APTmod_split_3x5_2),
  [_APT3] = LAYOUT_dilemma(APTv3_split_3x5_2),
  [_CAN] = LAYOUT_dilemma(Canary_split_3x5_2),
  [_NERPS] = LAYOUT_dilemma(Nerps_split_3x5_2),
  [_GAME] = LAYOUT_dilemma(GAMING_split_3x5_2),
  [_NAV] = LAYOUT_dilemma(NAV_split_3x5_2),
  [_SYM] = LAYOUT_dilemma(SYM_split_3x5_2),
  [_NUM] = LAYOUT_dilemma(NUMBER_split_3x5_2),
  [_NUMPD] = LAYOUT_dilemma(NUMPAD_split_3x5_2),
  [_FUN] = LAYOUT_dilemma(FUN_split_3x5_2),
  [_SPEC] = LAYOUT_dilemma(SPEC_split_3x5_2),
  [_MOUSE] = LAYOUT_dilemma(MOUSE_split_3x5_2),
};
// clang-format on
...
#define OLED_SLEEP_SPEED 40  // below this wpm value your animation will idle
...
#define OLED_KAKI_SPEED 100  // above this wpm value typing animation to triggere
...
bool oled_task_user(void) {
  ...
  if (display_splash_screen) {
    ...
    oled_write_P(PSTR("LCK"), /* inverted = */ led_usb_state.caps_lock);
    oled_write_P("\n", false);

    oled_write_P(PSTR("Layout: "), /* inverted = */ false);
    switch (get_highest_layer(default_layer_state)) {
        case _NRSTex:
            oled_write_P(PSTR("NRSTex"), /* inverted = */ false);
            break;
        case _CLMKdh:
            oled_write_P(PSTR("Colemak DH"), /* inverted = */ false);
            break;
        case _nAPTmak:
            oled_write_P(PSTR("APTmak"), /* inverted = */ false);
            break;
        case _ADEPT:
            oled_write_P(PSTR("Adept"), /* inverted = */ false);
            break;
        case _APTm:
            oled_write_P(PSTR("APT mod"), /* inverted = */ false);
            break;
        case _APT3:
            oled_write_P(PSTR("APTv3"), /* inverted = */ false);
            break;
        case _CAN:
            oled_write_P(PSTR("Canary"), /* inverted = */ false);
            break;
        case _GAME:
            oled_write_P(PSTR("Gamepad"), /* inverted = */ false);
            break;
        default:
            oled_write_P(PSTR("oops"), /* inverted = */ false);
            break;
    }
    oled_write_P("\n", false);
//    oled_write_P("\n\n", false);
